...
function BlogDescription({ blog }: Props) {
    ...
    () =>
      debounce(async (getValue) => {
        try {
          ...
          showToast(
            t("Sorry, an error occurred saving the blog", {
              type: "error",
            })
          );
          throw err;
  ...
  const key = isEditing || isDirty ? "draft" : blog.updatedAt;
    ...
    <MaxHeight data-editing={isEditing} data-expanded={isExpanded}>
      <Input data-editing={isEditing} data-expanded={isExpanded}>
        <span onClick={can.update ? handleStartEditing : undefined}>
          {blogs.isSaving && <LoadingIndicator />}
          {blog.hasDescription || isEditing || isDirty ? (
            <React.Suspense
              fallback={
                <Placeholder
                  onClick={() => {
...
export default observer(BlogDescription);
